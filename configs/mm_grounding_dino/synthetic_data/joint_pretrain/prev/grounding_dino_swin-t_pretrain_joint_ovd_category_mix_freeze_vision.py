...
ovd_phrase_dataset = dict(
    type='ODVGDataset',
    data_root='data/ovd_50k/',
    ann_file='annotations/train_odvg_phrase_grounding.json',
    label_map_file=None,
    data_prefix=dict(img='train/'),
    filter_cfg=dict(filter_empty_gt=True),
    pipeline=ovd_phrase_train_pipeline,
    return_classes=True,
    backend_args=None,
)
...
combined_dataset = dict(
    type='ConcatDataset',
    datasets=[ovd_phrase_dataset, ovd_category_dataset, flickr30k_dataset, obj365_dataset, gqa_dataset]
)

# You need to know or compute the number of samples in each dataset.
# For illustration, let’s say:
source_ratio = [5, 5, 1, 1, 1]
# source_ratio = [1]
batch_size = 16
